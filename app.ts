...
mostrarRespuestas();
console.log(`Palabra correcta: ${correcta}`); 
        ...
        if (/^[a-zA-Z]$/.test(event.key)) {
            if (correcta.includes(event.key)) {
                ...
                if (!DOM.fallos.textContent.includes(event.key)) {
                    ...
                    let intentosRestantes = parseInt(DOM.intentos.textContent) - 1;
...
function mostrarPalabra(palabra, ordenar = false) {
    let palabraMostrar = ordenar ? palabra : mezclarPalabra(palabra);
